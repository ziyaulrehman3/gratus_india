...
const productMenu = [
  { name: "Wood Table Lamp", desc: "Handcrafted wooden base, warm glow", href: "/products" },
  { name: "Pendant Lamp", desc: "Ceiling pendant, modern & minimal", href: "/products" },
  { name: "Moroccan Hanging Lamp", desc: "Moroccan jaali, ambient pattern light", href: "/products" },
  { name: "Rope Hanging Lamp", desc: "Jute rope, rustic hanging decor", href: "/products" },
  { name: "Wall & Hanging Planter Stand with Pot", desc: "Metal planter stand + pot, wall/hanging", href: "/products" },
  { name: "Floor Lamp", desc: "Tall floor lamp, living room statement", href: "/products" },
];
...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 py-2 text-[11.5px] leading-none">
          <div className="flex items-center gap-3 text-white/80">
            <span className="inline-flex items-center gap-1.5">
              ...
              <span className="font-semibold tracking-wide text-white">TRUSTED SINCE 1995</span>
              <span className="text-white/30">•</span>
              <span>Moradabad (UP) • Export-Quality Manufacturing</span>
            </span>
            <span className="hidden xl:inline-flex h-3 w-px bg-white/15" aria-hidden />
            <span className="hidden xl:inline-flex items-center gap-1.5 text-white/60">
              <span className="font-medium tracking-wide text-white/80">GST:</span>
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="tel:+919927683415" className="inline-flex items-center gap-1.5 font-medium text-white hover:text-[#ABD2FA] transition-colors">
            ...
            <a href="tel:+918791737818" className="hidden md:inline-flex text-white/60 hover:text-white transition-colors">
              +91 8791737818
            </a>
      ...
      <header
        ref={headerRef}
        className={`sticky top-0 z-40 will-change-transform transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          hidden ? "-translate-y-full" : "translate-y-0"
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-40 animate-shimmer" style={{ backgroundSize: "200% 100%" }} />
        {/* ambient glow */}
        <div
            ...
            {navLinks.map((link) => {
              ...
              return (
                <div
                  ...
                  onMouseLeave={() => {
                    if (isProducts) {
                      closeTimer.current = window.setTimeout(() => setProductOpen(false), 140) as unknown as number;
                    }
                  }}
                >
                  <Link
                  ...
                  >
                    ...
                    {!active && <span className="pointer-events-none absolute inset-x-3 -bottom-0.5 h-px scale-x-0 bg-gradient-to-r from-[#7692FF] to-[#ABD2FA] transition-transform duration-300 group-hover:scale-x-100" />}
                  </Link>
                      ...
                      <div className="relative overflow-hidden rounded-2xl bg-white p-2 shadow-[0_20px_60px_rgba(9,21,64,0.18),0_8px_20px_rgba(9,21,64,0.08)] ring-1 ring-gray-100">
                            ...
                            >
                              <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#091540] text-white shadow-sm transition-all group-hover/item:bg-[#1B2CC1] group-hover/item:scale-105">
                                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.7} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10m0-18v4" />
                                </svg>
                              </span>
                              <span>
                                <span className="block text-[13px] font-bold leading-tight text-[#091540] group-hover/item:text-[#1B2CC1]">{item.name}</span>
                                <span className="block text-xs leading-tight text-gray-500">{item.desc}</span>
                              </span>
                        ...
                        <div className="flex items-center justify-between gap-3 rounded-xl bg-gradient-to-br from-[#091540] via-[#0f1a5a] to-[#1B2CC1] px-4 py-3 text-white">
                          <div className="flex items-center gap-2 text-xs">
                            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-white/15 ring-1 ring-white/15">✓</span>
                            <div>
                              <div className="text-xs font-bold tracking-wide">Need custom specs?</div>
                              <div className="text-[11px] text-white/70">Share drawings • Quote in 24h • Lab reports</div>
                            </div>
                          </div>
                          <Link
                            href="/contact"
      ...
      {/* ===== Mobile Drawer ===== */}
      <div className={`fixed inset-0 z-50 lg:hidden ${menuOpen ? "visible" : "invisible"}`} aria-hidden={!menuOpen}>
        {/* Backdrop */}
        <div
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white p-1 shadow overflow-hidden"><img src="/logo.png" alt="Gratus India logo" className="h-full w-full object-contain" /></span>
              <div className="leading-none">
                <div className="text-sm font-extrabold tracking-[0.08em] text-white leading-none">GRATUS INDIA</div>
              </div>
            </div>
            <button
              onClick={() => setMenuOpen(false)}
          ...
          <div className="relative flex flex-1 flex-col overflow-y-auto px-6 py-6">
                ...
                return (
                  <div key={link.href}>
                    <Link
                      href={link.href}
                      onClick={() => setMenuOpen(false)}
                      style={{ transitionDelay: `${menuOpen ? i * 55 + 120 : 0}ms` }}
                    ...
                    {isProducts && (
                      <div
                        ...
                        style={{ transitionDelay: `${menuOpen ? i * 55 + 160 : 0}ms` }}
                      >
                        <div className="mt-2 ml-4 mr-1 grid grid-cols-1 gap-1.5 rounded-2xl bg-white/[0.05] p-2 ring-1 ring-white/10">
                          ...
                          <Link href="/products" onClick={() => setMenuOpen(false)} className="mt-1 flex items-center justify-center gap-1 rounded-full bg-white px-3 py-2 text-xs font-bold text-[#1B2CC1] hover:bg-[#f6f8ff]">
                            View all products →
                          </Link>
                        </div>
                      </div>
            ...
            <div className={`mt-6 rounded-2xl bg-gradient-to-br from-[#1B2CC1] via-[#1B2CC1] to-[#7692FF] p-[1.5px] transition-all duration-700 delay-300 ${menuOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`}>
              <div className="rounded-[15px] bg-[#091540] p-5">
                ...
                <p className="mt-2 text-sm leading-relaxed text-white/80">Custom specs? Get a 24h quote with lab reports & export docs. Trusted by 500+ clients.</p>
                <Link
                  href="/contact"
                  onClick={() => setMenuOpen(false)}
                ...
                <div className="mt-3 flex items-center justify-center gap-2 text-xs text-white/60">
                  <a href="tel:+919927683415" className="hover:text-white transition-colors font-medium">+91 9927683415</a>
                ...
                <div className="mt-2 flex items-center justify-center gap-2 text-[11px] text-white/50">
                  ...
                  <a href="mailto:gratusindia@gmail.com" className="hover:text-white">gratusindia@gmail.com</a>
                </div>
              </div>
            </div>
            ...
            <div className={`mt-auto pt-6 transition-all duration-700 delay-500 ${menuOpen ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
              <div className="flex items-center gap-2 text-[11px] font-semibold tracking-widest text-white/40">
                <span className="h-px flex-1 bg-white/10" /> PRECISION • CERTIFIED <span className="h-px flex-1 bg-white/10" />
              </div>
              <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                ...
                <span className="rounded-full bg-white/5 px-3 py-1 text-xs font-medium text-white/60 ring-1 ring-white/10">GST: 09BBZPA4353F2ZE</span>
              </div>
            </div>
          </div>
        </div>
